...
from .extend import (_container, primitive, notrace_primitive, VSpace, vspace,
...
@primitive
def capsule_take(A, idx):
    return A[idx]
def grad_capsule_take(ans, A, idx):
    return lambda g: capsule_untake(g, idx, vspace(A))
defvjp(capsule_take, grad_capsule_take)
defjvp(capsule_take, 'same')

class Sequence_container(_container):
    __slots__ = []
    __getitem__ = capsule_take
...
Sequence_container.register(tuple_)
Sequence_container.register(list_)

class Dict_container(_container):
    __slots__ = []
    __getitem__= capsule_take
    def __len__(self): return len(self._value)
...
Dict_container.register(dict_)

@primitive
def capsule_untake(x, idx, vs):
...
defvjp(capsule_untake, lambda ans, x, idx, _:
       lambda g: capsule_take(g, idx))
defjvp(capsule_untake, 'same')
...
def fwd_grad_make_sequence(argnum, g, ans, seq_type, *args, **kwargs):
    return capsule_untake(g, argnum-1, vspace(ans))
